...
test.describe('Remove Test Case from Test Group', () => {
  test('should remove a test case from a test group', async ({ page }) => {
    // Set a reasonable timeout
    test.setTimeout(120000);
    
    console.log('🔧 Starting Remove Test Case from Test Group test...');
    ...
    const dynamicName = await page.evaluate(() => {
      ...
      // Format: TestGroupRemoveTC-2025-12-31_20-10-00
      const dateTimestamp = `${year}-${month}-${day}_${hours}-${minutes}-${seconds}`;
      return `TestGroupRemoveTC-${dateTimestamp}`;
    });
    ...
    // Verify QAPA-31 is added before proceeding to removal
    ...
    const totalTextBefore = await page.getByText(/\d+ total/i).filter({ hasText: /total/i }).first().textContent();
    const totalCountBefore = totalTextBefore ? parseInt(totalTextBefore.match(/\d+/)?.[0] || '0') : 0;
    expect(totalCountBefore).toBe(1);
    console.log(`✅ Test Group Cases total before removal: ${totalCountBefore}`);
    
    // ============================================
    // SECTION 5: Remove Test Case from Group
    // ============================================
    console.log('📋 Section 5: Remove Test Case from Group');
    
    // Step 13: Select the checkbox for QAPA-31 in the Test Group Cases grid
    const qapa31Row = page.getByRole('row').filter({ hasText: 'QAPA-31' });
    await expect(qapa31Row).toBeVisible({ timeout: 90000 });
    
    const qapa31CheckboxInGrid = qapa31Row.getByRole('checkbox', { name: /Select row|Unselect row/i });
    await expect(qapa31CheckboxInGrid).toBeVisible({ timeout: 90000 });
    await qapa31CheckboxInGrid.click();
    console.log('✅ Selected checkbox for QAPA-31 in Test Group Cases grid');
    
    // Wait for checkbox to be selected
    await page.waitForTimeout(500);
    
    // Step 14: Click "Remove selected test case(s) from group" button
    const removeButton = page.getByRole('button', { name: /Remove selected test case/i });
    await expect(removeButton).toBeVisible({ timeout: 90000 });
    await removeButton.click();
    console.log('✅ Clicked Remove selected test case(s) from group button');
    
    // Wait for confirmation dialog to appear
    await page.waitForTimeout(1000);
    
    // Step 15: Verify confirmation dialog is visible
    await expect(page.getByRole('heading', { name: /Remove Test Cases/i })).toBeVisible({ timeout: 90000 });
    const confirmMessage = page.getByText(/Are you sure you want to remove/i);
    await expect(confirmMessage).toBeVisible({ timeout: 90000 });
    console.log('✅ Confirmation dialog is visible');
    
    // Step 16: Click Confirm button
    ...
    // Wait for removal to complete (3s after action)
    ...
    // Step 17: Verify QAPA-31 is removed from Test Group Cases
    console.log('🔍 Verifying QAPA-31 is removed from Test Group Cases...');
    
    // Verify test case count updated in Test Group Cases section
    const testGroupCasesSectionAfter = page.getByText(/Test Group Cases/i);
    await expect(testGroupCasesSectionAfter).toBeVisible({ timeout: 90000 });
    const totalTextAfter = await page.getByText(/\d+ total/i).filter({ hasText: /total/i }).first().textContent();
    const totalCountAfter = totalTextAfter ? parseInt(totalTextAfter.match(/\d+/)?.[0] || '0') : 0;
    expect(totalCountAfter).toBe(0);
    console.log(`✅ Test Group Cases total after removal: ${totalCountAfter}`);
    
    // Verify QAPA-31 is no longer visible in the Test Group Cases grid
    const qapa31InGrid = page.getByRole('row').filter({ hasText: 'QAPA-31' });
    const isQapa31Visible = await qapa31InGrid.isVisible({ timeout: 5000 }).catch(() => false);
    expect(isQapa31Visible).toBe(false);
    console.log('✅ QAPA-31 is no longer visible in Test Group Cases grid');
    
    // Verify "No rows" message is displayed
    const noRowsMessage = page.getByText(/No rows/i);
    const isNoRowsVisible = await noRowsMessage.isVisible({ timeout: 5000 }).catch(() => false);
    if (isNoRowsVisible) {
      console.log('✅ "No rows" message is displayed in the grid');
    }
    ...
    // Verify Test Cases by Module shows 0 Modules
    const modulesCountText = await page.getByText(/Test Cases by Module/i).locator('..').getByText(/\d+ Modules/i).first().textContent();
    const modulesCount = modulesCountText ? parseInt(modulesCountText.match(/\d+/)?.[0] || '0') : 0;
    expect(modulesCount).toBe(0);
    console.log(`✅ Test Cases by Module shows: ${modulesCount} Modules`);
    
    // Verify Total test cases shows 0
    const totalTestCasesText = await page.getByText(/Total: \d+ test cases/i).first().textContent();
    const totalTestCases = totalTestCasesText ? parseInt(totalTestCasesText.match(/\d+/)?.[0] || '0') : 0;
    expect(totalTestCases).toBe(0);
    console.log(`✅ Total test cases: ${totalTestCases}`);
    
    console.log('✅ Remove Test Case from Test Group test completed successfully');
  });
});
